...
import { Keychain, newAuthEntry } from '../keychain'
import Keyring from '../keyring'
import { AuthMap, NewAuthEntry, ThreeIDX } from '../three-idx'
...
describe('Keychain', () => {
  ...
  it('commit adds, IDX already created', async () => {
    ...
    await threeIdx.createIDX(
      await newAuthEntry(keychain._keyring, threeIdx.id, 'authid', randomAuthSecret())
    )
  ...
  it('commit removes', async () => {
    ...
    const keychain = new Keychain(keyring, threeIdx)
    await threeIdx.createIDX(
      await newAuthEntry(keychain._keyring, threeIdx.id, 'authid', authSecret0)
    )
